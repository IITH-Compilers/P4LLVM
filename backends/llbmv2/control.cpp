...
ControlConverter::handleTableImplementation(const IR::Property* implementation,
    ...
    if (propv->expression->is<IR::ConstructorCallExpression>()) {
        ...
        if (type_extern_name == LLBMV2::TableImplementation::actionProfileName) {
            table->emplace("type", "indirect");
        } else if (type_extern_name == LLBMV2::TableImplementation::actionSelectorName) {
            table->emplace("type", "indirect_ws");
        } else {
            ::error("%1%: unexpected type for implementation", dcltype);
            return false;
...
ControlConverter::convertTable(const CFG::TableNode* node,
    ...
    auto ctrs = table->properties->getProperty(LLBMV2::TableAttributes::countersName);
    if (ctrs != nullptr) {
        ...
        if (ctrs->value->is<IR::ExpressionValue>()) {
            ...
            if (expr->is<IR::ConstructorCallExpression>()) {
                ...
                if (te->name != LLBMV2::TableImplementation::directCounterName &&
                    te->name != LLBMV2::TableImplementation::counterName) {
                    ::error("%1%: Unexpected type %2% for property", ctrs, type);
                    return result;
                }
                ...
                bool direct = te->name == LLBMV2::TableImplementation::directCounterName;
    ...
    bool sup_to = false;
    auto timeout = table->properties->getProperty(LLBMV2::TableAttributes::supportTimeoutName);
    if (timeout != nullptr) {
    ...
    auto dm = table->properties->getProperty(LLBMV2::TableAttributes::metersName);
    if (dm != nullptr) {
        if (dm->value->is<IR::ExpressionValue>()) {
            ...
            if (!expr->is<IR::PathExpression>()) {
                ...
                auto te = type->to<IR::Type_Extern>();
                if (te->name != LLBMV2::TableImplementation::directMeterName) {
                    ::error("%1%: Unexpected type %2% for property", dm, type);
                    return result;
                }
                if (!decl->is<IR::Declaration_Instance>()) {
